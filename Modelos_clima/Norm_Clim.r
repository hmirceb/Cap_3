...
for (ii in 1:allcases) {
       mn.loglams[ii] <- mn.log.lam + 
         ...
         Plot_randomeffect[Plot_unif[ii]] + 
...
#This makes the annual lambda estimate, adding yr effect to mean lam estimate
  for(Year_iterator in 1:NYears) {
    ...
    annuallamest[Year_iterator] <- mn.log.lam + tmin_coefficient*tmin[Year_iterator] + 
...
#monitor#  mn.log.lam, process.loglam.sd, obs.error.sd, plot.sd, annuallamest, pcp_coefficient, tmax_coefficient, tmin_coefficient

#data#  Ndups, dupvars, logN,goodendrows,Ngoodendrows,allcases, lags, Plot_unif, Year, NYears, NPlots, pcp, pcp_all, tmax, tmax_all, tmin, tmin_all
